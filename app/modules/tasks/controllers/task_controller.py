...
async def create_service_request(
    ...
    # Email notifications (admin + primary ops mailbox)
    try:
        ...
        if admin_user:
            background_tasks.add_task(
                ...
                service_request.client_email,
                service_request.service_title,
                service_request.service_description,
                getattr(service_request.priority, 'value', str(service_request.priority)),
                service_request.created_at.isoformat() if hasattr(service_request.created_at, 'isoformat') else str(service_request.created_at)
            )
        # primary ops
        background_tasks.add_task(
            NotificationService().send_service_request_notification,
